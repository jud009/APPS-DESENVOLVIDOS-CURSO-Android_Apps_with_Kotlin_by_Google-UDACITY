...
class SleepQualityFragment : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        val dataBase = SleepDatabase.getInstance(application).dao
        val factory = SleepQualityViewModelFactory(args.sleepNightKey, dataBase)

        val viewModel = ViewModelProvider(this, factory).get(SleepQualityViewModel::class.java)
        binding.mViewModel = viewModel

        viewModel.navToTracker.observe(viewLifecycleOwner, Observer {
            if (it == true){
